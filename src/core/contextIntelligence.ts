...
export function detectContext(ctx: WindowContext): ScreenContext {
  ...
  // Terminal
  if (['cmd', 'powershell', 'windowsterminal', 'wt', 'conhost', 'gitbash', 'mintty', 'alacritty', 'hyper', 'cmder'].some(s => p.includes(s)))
    return 'terminal';
  if (['command prompt', 'powershell', 'terminal', 'bash', 'ubuntu', 'wsl'].some(s => t.includes(s)))
    return 'terminal';

  // Email desktop (olk = new Outlook, olk.exe; hxoutlook = Mail app)
  if (['outlook', 'olk', 'hxoutlook', 'thunderbird', 'mailbird', 'emclient'].some(s => p.includes(s)))
    return 'email-desktop';

  // Video conferencing
  if (['zoom', 'teams', 'webex', 'gotomeeting'].some(s => p.includes(s)))
    return 'video-call';
  if (['zoom meeting', 'microsoft teams', 'webex', 'google meet'].some(s => t.includes(s)))
    return 'video-call';

  // Messaging
  if (['whatsapp', 'telegram', 'slack', 'discord', 'signal', 'viber', 'skype'].some(s => p.includes(s)))
    return 'messaging';

  // Image editors / screenshot tools
  if (['mspaint', 'photoshop', 'gimp', 'irfanview', 'xnview', 'snagit', 'sharex', 'snippingtool', 'screensketch', 'lightshot'].some(s => p.includes(s)))
    return 'image-editor';
  ...
  // But first: if a browser is showing a local file, detect the file type instead
  if (isBrowser(p)) {
    // Local PDF/document opened in browser — detect as that file type, not as browser
    if (t.includes('.pdf')) return 'pdf-viewer';
    if (t.includes('.docx') || t.includes('.doc')) return 'word-processor';
    if (t.includes('.xlsx') || t.includes('.xls') || t.includes('.csv')) return 'spreadsheet';
    if (t.includes('.pptx') || t.includes('.ppt')) return 'presentation';
    // URL-encoded filenames: %20 in title + common PDF viewer keywords
    if (t.includes('%20') && (t.includes('paper') || t.includes('report') || t.includes('document'))) {
      // Likely a file:// URL with encoded spaces — check if it ends with a doc extension pattern
      if (/\.(pdf|docx?|xlsx?|pptx?)(%20|\s|$|-)/i.test(t)) return 'pdf-viewer';
    }
  ...
  // ── Last-resort title-based catch-all ──
  // Catches apps that use non-standard process names but have recognizable titles
  if (t.includes('outlook') || t.includes('- mail') || t.includes('inbox -'))
    return 'email-desktop';
  if (t.includes('visual studio code') || t.includes('- code'))
    return 'code-editor';
  if (t.includes('excel') || t.includes('.xlsx') || t.includes('.csv'))
    return 'spreadsheet';
  if (t.includes('word -') || t.includes('- word') || t.includes('.docx'))
    return 'word-processor';
  if (t.includes('powerpoint') || t.includes('.pptx'))
    return 'presentation';
  if (t.includes('- pdf') || t.includes('.pdf'))
    return 'pdf-viewer';
  if (t.includes('terminal') || t.includes('powershell') || t.includes('command prompt'))
    return 'terminal';
  if (t.includes('task manager'))
    return 'task-manager';
  if (t.includes('snipping tool') || t.includes('screenshot'))
    return 'image-editor';
...
// ============================================
// ACTION TYPE ENFORCEMENT (hybrid approach)
// ============================================
// Gemini generates actions online. We enforce types on the response.
// If Gemini returned a valid type, use it. Otherwise, infer from label.

export function enforceActionTypes(actions: any[], context: ScreenContext): ContextAction[] {
  return actions.map(action => {
    if (['chat', 'clipboard', 'document'].includes(action.type)) {
      return {
        label: action.label || 'Action',
        prompt: action.prompt || action.label,
        type: action.type as 'chat' | 'clipboard' | 'document',
        documentFormat: action.type === 'document' ? (action.documentFormat || guessDocFormat(context)) : undefined,
      };
    }
...
function guessDocFormat(context: ScreenContext): 'docx' | 'xlsx' | 'pdf' {
  // Smart format: don't suggest the same format the user already has open
  if (context === 'spreadsheet' || context === 'supply-chain') return 'pdf'; // Already in Excel → suggest PDF
  if (context === 'word-processor') return 'pdf'; // Already in Word → suggest PDF
  if (context === 'pdf-viewer') return 'docx'; // Already in PDF → suggest editable Word
  return 'docx';
}
...
// Full ContextInsight type — matches gemini.ts interface
interface CachedContextInsight {
  seeing: string;
...
export function getInsightCacheKey(context: ScreenContext, windowTitle: string): string {
  const normalized = windowTitle
    .replace(/\s*[-–—]\s*\d{1,2}:\d{2}(:\d{2})?\s*(AM|PM)?/gi, '')
    .replace(/\s*\*\s*$/, '')
    .trim()
    .toLowerCase();
...
export function getCachedInsight(cacheKey: string): CachedContextInsight | null {
...
export function setCachedInsight(cacheKey: string, data: CachedContextInsight, context: ScreenContext): void {
  insightCache = { key: cacheKey, data, context, timestamp: Date.now() };
}
...
const CONTEXT_ACTIONS: Record<ScreenContext, ContextAction[]> = {
  ...
  'spreadsheet': [
    { label: 'Explain Visible Data', prompt: 'Describe what is visible in this spreadsheet: column headers, data types, and apparent purpose. Only reference what you can see.', type: 'chat' },
    { label: 'Find Visible Errors', prompt: 'Identify any formula errors, broken references, or issues visible on screen.', type: 'chat' },
    { label: 'Copy Visible Data', prompt: 'Extract the visible column headers and key data values. Return ONLY the data as structured text.', type: 'clipboard' },
  ],

  'word-processor': [
    { label: 'Explain This Section', prompt: 'Describe what is visible in this document section. Only reference content you can see.', type: 'chat' },
    { label: 'Fix Visible Grammar', prompt: 'Identify grammatical and spelling errors in the visible text. Explain each fix.', type: 'chat' },
    { label: 'Copy Corrected', prompt: 'Rewrite the visible text with grammar and spelling fixed. Return ONLY the corrected text.', type: 'clipboard' },
  ],
  ...
  'pdf-viewer': [
    { label: 'Explain This Page', prompt: 'Explain what is visible on this PDF page in simple language. Only reference content you can see.', type: 'chat' },
    { label: 'Copy Visible Text', prompt: 'Extract all readable text from the visible portion of this PDF page. Return ONLY the text.', type: 'clipboard' },
    { label: 'Describe Content', prompt: 'Describe the structure and content visible on this page: headings, paragraphs, tables, images.', type: 'chat' },
  ],
  ...
  'browser-general': [
    { label: 'Explain Visible', prompt: 'Describe what is visible on this web page. Only reference content you can see.', type: 'chat' },
    { label: 'Copy Visible Text', prompt: 'Extract all readable text visible on screen. Return ONLY the text.', type: 'clipboard' },
    { label: 'Search Topic', prompt: 'Based on the visible page topic, suggest related search queries and key terms.', type: 'chat' },
  ],
...
// ============================================
// FILE ACCESS DETECTION & FULL-DOC ACTIONS
// ============================================

const FILE_ACCESSIBLE_CONTEXTS = new Set<ScreenContext>([
  'spreadsheet', 'word-processor', 'presentation', 'pdf-viewer',
  'notepad', 'collaboration-docs',
  // code-editor removed: readActiveFile only supports .pdf/.docx/.xlsx/.pptx/.txt
  // Code files (.ts/.js/.py) are not readable — and the background process scan
  // would pick up minimized PDFs instead of the actual code file.
]);

export function isFileAccessibleContext(context: ScreenContext): boolean {
  return FILE_ACCESSIBLE_CONTEXTS.has(context);
}

/** Infer ScreenContext from a filename extension (fallback when context detection fails) */
export function inferContextFromFileName(fileName: string): ScreenContext | null {
  const ext = fileName.toLowerCase().replace(/^.*\./, '.');
  if (ext === '.xlsx' || ext === '.xls' || ext === '.csv') return 'spreadsheet';
  if (ext === '.docx' || ext === '.doc') return 'word-processor';
  if (ext === '.pptx' || ext === '.ppt') return 'presentation';
  if (ext === '.pdf') return 'pdf-viewer';
  if (ext === '.txt' || ext === '.md' || ext === '.rtf') return 'notepad';
  return null;
}

// Full-document actions — shown when file content has been successfully read.
// Rules: No same-format document generation (no "Export to Excel" in Excel, no "Save as PDF" in PDF).
// Clipboard actions should be genuinely useful (key metrics, summaries, contacts — not raw data dumps).
// Always include at least one action that leverages the FULL content even if the visible screen is empty/unhelpful.
const CONTEXT_ACTIONS_FULL: Partial<Record<ScreenContext, ContextAction[]>> = {

  'spreadsheet': [
    { label: 'Analyze All Data', prompt: 'Analyze the full spreadsheet across all sheets: key metrics, trends, outliers, and actionable insights.', type: 'chat' },
    { label: 'Copy Key Metrics', prompt: 'Extract the most important metrics, totals, and KPIs from all sheets. Format as a clean summary with labels and values. Return ONLY the data.', type: 'clipboard' },
    { label: 'Generate Report', prompt: 'Generate a comprehensive analysis report from all spreadsheet data with findings and recommendations.', type: 'document', documentFormat: 'docx' },
  ],

  'word-processor': [
    { label: 'Summarize Document', prompt: 'Provide a comprehensive summary of the entire document: main topics, key arguments, conclusions.', type: 'chat' },
    { label: 'Copy Key Points', prompt: 'List all key points, findings, and conclusions from the full document as bullet points. Return ONLY the key points.', type: 'clipboard' },
    { label: 'Check Grammar', prompt: 'Review the entire document for grammar, spelling, punctuation, and style issues. List each issue with the correction.', type: 'chat' },
  ],

  'presentation': [
    { label: 'Summarize All Slides', prompt: 'Provide an overview of the entire presentation: key messages per slide, overall narrative arc.', type: 'chat' },
    { label: 'Copy Slide Content', prompt: 'Extract all text content from every slide, organized by slide number. Return ONLY the content.', type: 'clipboard' },
    { label: 'Review Full Deck', prompt: 'Provide feedback on the entire presentation: structure, flow, completeness, and design consistency.', type: 'chat' },
  ],

  'pdf-viewer': [
    { label: 'Summarize Document', prompt: 'Summarize the entire PDF document: main topics, key information, and conclusions.', type: 'chat' },
    { label: 'Copy Key Data', prompt: 'Extract all key data points, names, dates, amounts, and findings from the entire document. Return ONLY the extracted data as structured text.', type: 'clipboard' },
    { label: 'Convert to Word', prompt: 'Convert the full PDF content into a well-formatted Word document preserving structure.', type: 'document', documentFormat: 'docx' },
  ],

  'code-editor': [
    { label: 'Analyze Full File', prompt: 'Review the entire file: architecture, patterns, potential issues, and improvement suggestions.', type: 'chat' },
    { label: 'Find All Bugs', prompt: 'Scan the entire file for bugs, security issues, and bad practices. List each with severity.', type: 'chat' },
    { label: 'Copy Summary', prompt: 'Describe what this file does: purpose, exports, key functions. Return ONLY a concise summary.', type: 'clipboard' },
  ],

  'notepad': [
    { label: 'Summarize Content', prompt: 'Summarize the entire text content.', type: 'chat' },
    { label: 'Copy Formatted', prompt: 'Clean up, structure, and format the entire text content. Return ONLY the formatted version.', type: 'clipboard' },
    { label: 'Convert Format', prompt: 'Convert the full content to a structured format (JSON, CSV, or Markdown as appropriate). Return ONLY the converted text.', type: 'clipboard' },
  ],

  'collaboration-docs': [
    { label: 'Summarize Full Doc', prompt: 'Comprehensive summary of the entire document with key decisions and action items.', type: 'chat' },
    { label: 'Copy Action Items', prompt: 'List all action items, decisions, and deadlines from the full document. Return ONLY the items.', type: 'clipboard' },
    { label: 'Export Summary', prompt: 'Generate a summary document with key points and action items.', type: 'document', documentFormat: 'docx' },
  ],

  'browser-general': [
    { label: 'Summarize Full Page', prompt: 'Summarize the entire web page content, not just the visible portion.', type: 'chat' },
    { label: 'Copy Main Content', prompt: 'Extract the main article/content from the full page, excluding navigation and ads. Return ONLY the clean text.', type: 'clipboard' },
    { label: 'Save as Word', prompt: 'Convert the full page content to a formatted Word document.', type: 'document', documentFormat: 'docx' },
  ],

  'browser-news': [
    { label: 'Full Article Summary', prompt: 'Summarize the complete article: key facts, arguments, and conclusions.', type: 'chat' },
    { label: 'Copy Key Facts', prompt: 'Extract all key facts, quotes, and figures from the full article. Return ONLY the facts as bullet points.', type: 'clipboard' },
    { label: 'Save Article', prompt: 'Convert the full article to a Word document.', type: 'document', documentFormat: 'docx' },
  ],

  'browser-dev': [
    { label: 'Full Solution', prompt: 'Explain the complete solution from the page including all code examples and explanations.', type: 'chat' },
    { label: 'Copy All Code', prompt: 'Extract all code snippets from the page. Return ONLY the code blocks.', type: 'clipboard' },
    { label: 'Summarize Thread', prompt: 'Summarize the entire discussion: problem, solutions tried, and accepted answer.', type: 'chat' },
  ],

  'browser-email': [
    { label: 'Full Email Summary', prompt: 'Summarize the complete email thread: all messages, key points, and action items.', type: 'chat' },
    { label: 'Copy All Tasks', prompt: 'List every action item and deadline from the entire email thread. Return ONLY the tasks.', type: 'clipboard' },
    { label: 'Copy Draft Reply', prompt: 'Draft a thorough reply addressing all points in the email. Return ONLY the reply text.', type: 'clipboard' },
  ],

  'browser-learning': [
    { label: 'Summarize Lesson', prompt: 'Complete summary of the lesson content with key concepts and examples.', type: 'chat' },
    { label: 'Copy Study Notes', prompt: 'Generate comprehensive study notes from the entire lesson. Return ONLY the notes.', type: 'clipboard' },
    { label: 'Quiz Me', prompt: 'Create 10 practice questions covering the entire lesson content with answers.', type: 'chat' },
  ],

  'browser-jobs': [
    { label: 'Full Job Analysis', prompt: 'Complete analysis of the job posting: role, requirements, benefits, and company info.', type: 'chat' },
    { label: 'Copy Cover Letter', prompt: 'Create a cover letter addressing all requirements in the posting. Return ONLY the letter.', type: 'clipboard' },
    { label: 'Copy Job Details', prompt: 'Extract all job details: title, company, requirements, salary, benefits. Return ONLY the data.', type: 'clipboard' },
  ],

  'browser-shopping': [
    { label: 'Full Product Review', prompt: 'Complete product analysis: all specs, reviews summary, pricing, and value assessment.', type: 'chat' },
    { label: 'Copy All Specs', prompt: 'Extract all product specifications, price, and key details. Return ONLY the data.', type: 'clipboard' },
    { label: 'Compare Options', prompt: 'Summarize the visible products and suggest which is the best value.', type: 'chat' },
  ],
};

export function getContextActionsForAccess(context: ScreenContext, hasFullAccess: boolean): ContextAction[] {
  if (hasFullAccess) {
    const fullActions = CONTEXT_ACTIONS_FULL[context];
    if (fullActions) return fullActions;
  }
  // Fallback to screenshot-only actions
  return CONTEXT_ACTIONS[context] || CONTEXT_ACTIONS['unknown'];
}

...
export function Breadcrumbs() {
    ...
    return (
        ...
        >
            <div
                className="flex items-center gap-1 px-3 py-1.5 rounded-full bg-[#12121a]/90 backdrop-blur-xl border border-emerald-500/20 shadow-lg text-[11px] text-white/70 font-[Thmanyah_Sans,system-ui,sans-serif]"
            >
                <button
                    onClick={(e) => { e.stopPropagation(); dispatch({ type: 'SET_FOCUSED_CONTAINER', id: null }); }}
                    title={t('canvas.breadcrumb_exit_root')}
                    className="flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-emerald-500/15 hover:text-emerald-200 transition-colors"
                >
                    <Home size={11} />
                    <span>{t('canvas.breadcrumb_root')}</span>
                </button>
                {chain.map((c, i) => {
                    ...
                    return (
                        <React.Fragment key={c.id}>
                            ...
                            <button
                                ...
                                title={isLast ? t('canvas.breadcrumb_you_are_here') : t('canvas.breadcrumb_focus_into').replace('{name}', c.title)}
                            ...
                            >
                                {c.title || t('canvas.group_untitled')}
                            </button>
                        </React.Fragment>
                    );
                })}
